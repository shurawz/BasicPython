...
class Scrollbox(tkinter.Listbox):  # "tkinter.Scrollbox" ko subclass ho hai guyz.
...
def get_albums(event):
    ...
    artist_id = db.execute("SELECT artists._id FROM artists WHERE artists.name=?", artist_name).fetchone()
    alist = []
    for row in db.execute("SELECT albums.name FROM albums WHERE albums.artist = ? ORDER BY albums.name", artist_id):
        alist.append(row[0])
...
def get_songs(event):
    ...
    # get the artist ID from the database row
    album_id = db.execute("SELECT albums._id FROM albums WHERE albums.name=?", album_name).fetchone()
    alist = []
    for x in db.execute("SELECT songs.title FROM songs WHERE songs.album = ? ORDER BY songs.track", album_id):
        alist.append(x[0])
